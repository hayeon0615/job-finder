...
export default function Home() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error("Supabase environment variables are missing", error);
      void Promise.resolve().then(() => {
        if (!mounted) return;
        setLoadError("Supabase 연결 설정이 없어 공고를 불러오지 못했어요.");
        setIsLoading(false);
      });
      return () => {
        mounted = false;
    ...
    async function loadJobs() {
      ...
      if (error) {
        ...
        setLoadError("공고를 불러오지 못했어요. 잠시 후 다시 시도해주세요.");
        setJobs([]);
      } else {
        setLoadError("");
